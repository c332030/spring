package c332030.utils.comm.email;
...
public class EmailUtils {
     ...
     * @return
     */
    public static final boolean isEmail(String emailAddress) {
        if(Tools.isEmpty(emailAddress)
                || emailAddress.contains(" ")
        ) {
            return false;
        }

        String[] emailStrs = emailAddress.split("@");

        if(2 != emailStrs.length
                || Tools.isEmpty(emailStrs[0].trim())
                || Tools.isEmpty(emailStrs[1].trim())
        ) { // 邮件地址以 @ 分割
            return false;
        }


        return emailStrs[1].split("\\.").length >= 2;
    }
     ...
     */
    /*
    public static boolean sendEmail25(String subject, String[] toUsers, String[] ccUsers, String content, List<Map<String, String>> attachfiles) {
        boolean flag = true;
        try {
            JavaMailSenderImpl javaMailSender = new JavaMailSenderImpl();
            javaMailSender.setHost(host);
